...
export class PluginManager extends EventEmitter {
  ...
  async unloadPlugin(name: string): Promise<void> {
    ...
    try {
      // Mark plugin as invalidated first to prevent context usage during cleanup
      this.invalidatedPlugins.add(name);

      // Call plugin cleanup if provided
      if (plugin.cleanup) {
      ...
      for (const file of pluginFiles) {
        ...
        if (stats.isFile()) {
          try {
            ...
            console.error(`Failed to parse plugin ${file}:`, err.message);
          }
        }
      }
